...
public class GUIBan implements CommandExecutor {

	private BanGUI plugin;

	public GUIBan(BanGUI plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String name, String[] args) {
		if (cmd.getName().equalsIgnoreCase("GUIBan")) {
			if (sender instanceof Player) {
				if (sender.hasPermission("ban.gui")) {
					if (args.length == 1) {
						if (Bukkit.getOfflinePlayer(args[0]) != null) {
							if (!Bukkit.getOfflinePlayer(args[0]).isBanned()) {
								...
								plugin.getBanning().put(banning, new BanInfo(args[0], false));
						...
						sender.sendMessage(ChatColor.RED + "Usage: /GUIBan (name)");
					}
				} else {
					sender.sendMessage(ChatColor.RED + "Missing permission: ban.gui");
				}
			} else {
